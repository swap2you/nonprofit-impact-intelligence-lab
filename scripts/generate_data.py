...
def gen():
 ...
 programs=[(i+1,f"PRG-{i+1:02d}",n) for i,n in enumerate(["Learning Access","Community Health","Youth Leadership","Digital Inclusion","Climate Resilience","School Progress"])]
 ...
 for s in sites:
  ...
  for p in periods:
   ...
   if s['site_id']==15 and idx==6: issues.append(dict(issue_id=len(issues)+1,table_name='fact_program_outcomes',record_id=rid,dimension='consistency',issue_type='key_measure_mismatch',severity='Medium',description='Outcome key does not reconcile to enrollment snapshot',detected_at=now))
   if s['site_id']==21 and idx==7: issues.append(dict(issue_id=len(issues)+1,table_name='fact_enrollment',record_id=rid,dimension='uniqueness',issue_type='duplicate_source_key',severity='High',description='Duplicate source key detected during profiling',detected_at=now))
   if s['site_id']==30 and idx==8: issues.append(dict(issue_id=len(issues)+1,table_name='fact_data_submission',record_id=rid,dimension='timeliness',issue_type='late_submission',severity='Low',description='Submission arrived after reporting cutoff',detected_at=now))
   rid+=1
