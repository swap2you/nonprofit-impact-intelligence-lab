...
from analytics.methods import trend,diagnostic,bounded_forecast
...
@app.get('/analytics')
def analytics():
 t=trend(q('select period_id,enrolled from fact_enrollment')); ch=float(t['pct_change'].iloc[-1]) if len(t) else 0.0; stale=float(q('select avg(case when is_stale then 1.0 else 0.0 end) x from fact_data_submission').iloc[0,0]); rows=[{'period_id':int(r.period_id),'enrolled':float(r.enrolled),'rolling_3':float(r.rolling_3),'pct_change':float(r.pct_change)} for r in t.itertuples()]; return {'trend':rows,'diagnostic':diagnostic(ch,stale,.03),'stale_rate':stale,'forecast':bounded_forecast(q('select period_id,enrolled from fact_enrollment'))}
@app.get('/migration')
def migration():
